...
By contributing to Sculk Spread API, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Sculk Spread API.
...
It allows to separate the actual name of the variable and the namespace, `sculkSpreadApi`.

Fields marked as `@Unique` must be prefixed with `sculkSpreadApi` or `SCULKSPREADAPI` if the field is static and final.
...
[LICENSE]: ./LICENSE "Sculk Spread API license file"
